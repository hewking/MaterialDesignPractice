...
class PageController extends BaseController {
    ...
    public function index(){
        ...
        $page_param = M('page_param')->where(array('pageid'=>$page_id))->find();

        if($page_param){
            $param_arr = explode(',', $page_param['param']);
            $value_arr = explode(',', $page_param['value']);
            $result = array();
            for($i = 0;$i < count($param_arr);$i++){
                $result[$i]['params'] = $param_arr[$i];
                $result[$i]['values'] = $value_arr[$i];
            }
