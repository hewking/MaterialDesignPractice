          ...
          <!-- dropdown menu links -->
            <li><a href="#" id="share">分享</a></li>
             ...
             <?php if($ItemCreator): ?><li><a href="<?php echo U('Home/Item/add');?>?item_id=<?php echo ($item["item_id"]); ?>">修改信息</a></li>          
              ...
              <li><a href="<?php echo U('Home/Item/delete');?>?item_id=<?php echo ($item["item_id"]); ?>">删除</a></li>
			  <li><a href="<?php echo U('Home/Item/copyItem');?>?item_id=<?php echo ($item["item_id"]); ?>">复制项目</a></li><?php endif; ?>
  ...
  <!-- 左侧栏菜单 -->
      ...
      <ul class="nav nav-list bs-docs-sidenav">
        ...
        <?php if(is_array($catalogs)): foreach($catalogs as $key=>$catalog): ?><li><a href="#"><i class="icon-chevron-right"></i><?php echo ($catalog["cat_name"]); ?></a>
            <ul class="child-ul nav-list hide">
              <?php if(is_array($catalog["child"])): foreach($catalog["child"] as $key=>$child): ?><li><a href="#"><i class="icon-chevron-right"></i><?php echo ($child["cat_name"]); ?></a>
                <ul class="child-ul nav-list hide">
                  <?php if(is_array($child["pages"])): foreach($child["pages"] as $key=>$child_page): ?><li><a href="<?php echo U('Home/Page/index');?>?page_id=<?php echo ($child_page["page_id"]); ?>" data-page-id="<?php echo ($child_page["page_id"]); ?>" ><?php echo ($child_page["page_title"]); ?></a></li><?php endforeach; endif; ?>
                </ul>
    ...
    <div class="doc-right  span12 ">
      ...
      <div class='page-edit-link pull-right hide'>
        <ul class="inline">
          <?php if($ItemPermn): ?><li><a id="btn-param">测试参数</a></li>
...
<!--添加测试参数 -->
<div class="modal hide fade" id="test-param">
  <div class="modal-header">
    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
    <h3>测试参数</h3>
  </div>
  <div class="control-group">
    <form id="form1" action="<?php echo U('Item/param_add');?>" method="post"> 
...
$(function(){
  ...
  $("#btn-param").click(function(){
      var htmlstr = $(window.frames["main"].document).find("#sometable").html();
      $("#form1").html(htmlstr);
  })
  ...
  $("#btnok").click(function(){
      ...
      //var page_id= $(window.frames["main"].document).find("#pageid").val();
      //$("#pageid").val(page_id);
      //$("#form1").submit();
  });
})
